...
case class Canvas() extends Component {
  ...
  def +=(r: Rectangle) = add { graphic =>
    ...
    r match {
      ...
      case RoundRectangle(_, _, _, arcWidth, arcHeight) =>
        if(filled) graphic.fillRoundRect(x, y, width, height, arcWidth, arcHeight)
        else graphic.drawRoundRect(x, y, width, height, arcWidth, arcHeight)
      case _=> ()
    }
  }
   ...
   * @param pic : Picture
   */
  def +=(pic: Picture) = add (pic.draw(_))
